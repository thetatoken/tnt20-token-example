...
const getContext = (network) => {
    let web3
    let chainID
    
    if (network == "ganache") {
        web3 = new Web3('http://127.0.0.1:8545')
        chainID = 3456 // for the local simulated network
    } else if (network == "theta_testnet") {
        web3 = new Web3('https://eth-rpc-api-testnet.thetatoken.org/rpc')
        chainID = 365 // for the Theta Testnet
    } else if (network == "theta_mainnet") {
        web3 = new Web3('https://eth-rpc-api.thetatoken.org/rpc')
        chainID = 361 // for the Theta Mainnet
    } else {
        console.log("Network", network, "not supported yet")
        exit(1)
    }
